...
const columns: GridColDef[] = [
  {
    ...
    renderCell: (params: any) => {
      ...
      return (
        <Link to={`/orders/${value}`} className="text-primary text-decoration-none">
          {value}
        </Link>
      );
    },
  ...
  },
  {
    ...
    headerAlign: 'center',
    align: 'center',
    valueFormatter: ({ value }: any) => (value ? new Date(value as string | number | Date).toLocaleString() : ''),
  },
];
...
function Analytics() {
  ...
  useEffect(() => {
    const fetchOrders = async () => {
      ...
      try {
        ...
        const response = await api.get('/orders', { params }) as { orders: Order[]; total: number };
        setRows(response.orders || []);
        setRowCount(response.total ?? response.orders.length);
      } catch (fetchError) {
        const message = fetchError instanceof Error ? fetchError.message : 'Unable to load analytic orders.';
        setError(message);
      } finally {
        setLoading(false);
      }
  ...
  const sortModel: GridSortModel = [{ field: sortField, sort: sortOrder === 'asc' ? 'asc' : 'desc' }];
