...
function Notifications() {
  const lowStockProducts = initialProducts.filter(
    (product: Product) => product.inventory > 0 && product.inventory <= LOW_STOCK_THRESHOLD,
  );
  const outOfStockProducts = initialProducts.filter((product: Product) => product.inventory === 0);
  const recentOrders = [...initialOrders]
    .sort((a: Order, b: Order) => new Date(b.createdAt).getTime() - new Date(a.createdAt).getTime())
    .slice(0, 5);

  const formatDateTime = (isoString: string): string => new Date(isoString).toLocaleString();
