...
import type { Order, OrderStatus, PaymentStatus } from '../types/ecommerce';
...
function EditOrder() {
  ...
  const existingOrder = items.find((item: Order) => item.id === orderId);
  const [order, setOrder] = useState<{ customerName: string; total: string; quantity: number; status: OrderStatus; paymentStatus: PaymentStatus; createdAt: string }>({
    customerName: '',
    total: '',
    quantity: 1,
    status: 'Pending',
  ...
  useEffect(() => {
    if (existingOrder) {
      setOrder({
        ...
        total: String(existingOrder.total),
  ...
  const handleChange = (event: ChangeEvent<HTMLInputElement | HTMLSelectElement>) => {
    ...
    setOrder((prev) => ({
      ...prev,
      [name as keyof typeof prev]: name === 'quantity' ? Number(value) : value,
    }));
  };
