...
const mapOrdersByQuarter = (orders = []) => {
...
const mapOrderStatusData = (orders = []) => {
...
const mapRevenueByPeriod = (orders = []) => {
  const now = new Date();
...
const mapRevenueTrendData = (orders = []) => {
  const now = new Date();
  const startOfToday = new Date(now);
  startOfToday.setHours(0, 0, 0, 0);
  ...
  orders.forEach((order) => {
    ...
    if (createdAt >= startOfWeek) {
      const dayIndex = Math.floor((createdAt - startOfWeek) / (1000 * 60 * 60 * 24));
    ...
    if (createdAt >= startOfMonth) {
      const dayIndex = Math.floor((createdAt - startOfMonth) / (1000 * 60 * 60 * 24));
...
const mapOrdersCountByPeriod = (orders = []) => {
  const now = new Date();
  const startOfToday = new Date(now);
  startOfToday.setHours(0, 0, 0, 0);
...
const mapOrdersCountTrendData = (orders = []) => {
  const now = new Date();
  const startOfToday = new Date(now);
  startOfToday.setHours(0, 0, 0, 0);
...
const mapCategoryPortfolio = (products = []) => {
...
const mapTopProducts = (products = [], orders = []) => {
...
const buildDashboardMetrics = (products = [], orders = []) => {
...
export default function useDashboardAnalytics(products = [], orders = []) {
  ...
  const recentProducts = useMemo(() => filteredProducts, [filteredProducts]);
