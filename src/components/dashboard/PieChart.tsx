...
function PieChart({ data, colors, label }: PieChartProps) {
  const total = data.reduce((sum: number, item: ChartDatum) => sum + item.value, 0) || 1;
  let angle = 0;

  const segments = data.map((item: ChartDatum, index: number) => {
