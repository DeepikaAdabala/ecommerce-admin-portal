...
function BarChart({ data, label, params = [], color = '#673bf681', isVertical = false, xAxisLabel = '', yAxisLabel = '' }: BarChartProps) {
  const maxValue = Math.max(...data.map((item: ChartDatum) => item.value), 1);
    ...
    return (
      <div className="chart-card">
        ...
        <svg viewBox={`0 0 ${width} ${height}`} className="w-100">
          ...
          {data.map((item: ChartDatum, index: number) => {
